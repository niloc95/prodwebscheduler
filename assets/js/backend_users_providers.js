...
(function () {
    ...
    ProvidersHelper.prototype.bindEventHandlers = function () {
        ...
        $('#providers').on('click', '#delete-provider', function () {
            ...
            GeneralFunctions.displayMessageBox(WBLang.delete_provider,
                WBLang.delete_record_prompt, buttons);
        }.bind(this));
    ...
    ProvidersHelper.prototype.save = function (provider) {
        ...
        $.post(postUrl, postData, function (response) {
            ...
            Backend.displayNotification(WBLang.provider_saved);
            this.resetForm();
            $('#filter-providers .key').val('');
    ...
    ProvidersHelper.prototype.delete = function (id) {
        ...
        $.post(postUrl, postData, function (response) {
            ...
            Backend.displayNotification(WBLang.provider_deleted);
            this.resetForm();
    ...
    ProvidersHelper.prototype.validate = function () {
        ...
        try {
            ...
            if ($('#provider-password').val() != $('#provider-password-confirm').val()) {
                ...
                throw WBLang.passwords_mismatch;
            }

            if ($('#provider-password').val().length < BackendUsers.MIN_PASSWORD_LENGTH
                && $('#provider-password').val() != '') {
                $('#provider-password, #provider-password-confirm').closest('.form-group').addClass('has-error');
                throw WBLang.password_length_notice.replace('$number', BackendUsers.MIN_PASSWORD_LENGTH);
            }
            ...
            if ($('#provider-username').attr('already-exists') == 'true') {
                ...
                throw WBLang.username_already_exists;
            }
    ...
    ProvidersHelper.prototype.filter = function (key, selectId, display) {
        ...
        $.post(postUrl, postData, function (response) {
            ...
            if (response.length == 0) {
                $('#filter-providers .results').html('<em>' + WBLang.no_records_found + '</em>')
            }
    ...
    ProvidersHelper.prototype.editableBreakDay = function ($selector) {
        var weekDays = {};
        weekDays[WBLang.monday] = 'Monday';
        weekDays[WBLang.tuesday] = 'Tuesday';
        weekDays[WBLang.wednesday] = 'Wednesday';
        weekDays[WBLang.thursday] = 'Thursday';
        weekDays[WBLang.friday] = 'Friday';
        weekDays[WBLang.saturday] = 'Saturday';
        weekDays[WBLang.sunday] = 'Sunday';
