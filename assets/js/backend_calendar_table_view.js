...
(function (exports) {
        ...
        $calendar.on('click', '.event', function (event) {
            ...
            if ($(this).hasClass('unavailability')) {
                ...
                html =
                    ...
                    + '<hr>' +
                    '<center>' +
                    '<button class="edit-popover btn btn-primary">' + WBLang.edit + '</button>' +
                    '<button class="delete-popover btn btn-danger">' + WBLang.delete + '</button>' +
                    '<button class="close-popover btn btn-default" data-po=' + event.target + '>' + WBLang.close + '</button>' +
                    '</center>';
            } else {
                html =
                    '<style type="text/css">'
            ...
            var title = entry.is_unavailable !== '0' ? WBLang.unavailable : entry.service.name + ' - '
    ...
    function _createHeader() {
        ...
        $calendarFilter
            .find('select')
            .empty()
            .append(new Option('1 ' + WBLang.day, 1))
            .append(new Option('3 ' + WBLang.days, 3));
    ...
    function _createSlots($providerColumn, date, provider) {
        ...
        if (!plan) {
            $providerColumn.append('<div class="not-working">'
                + (provider.first_name + ' ' + provider.last_name).trim() + ' <br> ' + WBLang.not_working
                + '</div>');
            return;
        }
    ...
    function _createUnavailabilities($providerColumn, unavailabilities) {
        ...
        for (var index in unavailabilities) {
            ...
            $event.html((unavailability.notes || WBLang.unavailable) +
