...
(function(exports) {
    ...
    exports.initialize = function(bindEventHandlers, manageMode) {
        ...
        if (GlobalVariables.displayCookieNotice) {
            cookieconsent.initialise({
                palette: {
                    popup: {
                        background:  'rgba(0,0,0,0.76)', //'#ffffffbd',
                        text: '#ffffff'
                    },
                    button: {
                        //background: '#3DD481',
                ...
                content: {
                    message: WBLang.website_using_cookies_to_ensure_best_experience,
                    dismiss: 'ACCEPT'
                },
            });
            ...
            $('.cc-link').replaceWith(
                $('<a/>', {
                    ...
                    'href': 'https://webscheduler.co.za/privacy-policy/',
                    'class': 'cc-link',
                    'text': $('.cc-link').text()
                })
            );
        ...
        $('#select-date').datepicker({
            ...
            dayNames: [
                WBLang.sunday, WBLang.monday, WBLang.tuesday, WBLang.wednesday,
    ...
    function _bindEventHandlers() {
        ...
        $('#select-service').change(function() {
            ...
            if ($('#select-provider option').length >= 1) {
                $('#select-provider').append(new Option('- ' + WBLang.any_provider + ' -', 'any-provider'));
            }
                ...
                if ($('.selected-hour').length == 0) {
                    if ($('#select-hour-prompt').length == 0) {
                        $('#available-hours').append('<br><br>' +
                            ...
                            WBLang.appointment_hour_missing +
                            '</span>');
                    }
                    return;
                }
        ...
        if (FrontendBook.manageMode) {
            ...
            $('#cancel-appointment').click(function(event) {
                ...
                GeneralFunctions.displayMessageBox(WBLang.cancel_appointment_title,
                    WBLang.write_appointment_removal_reason, buttons);
            ...
            $('#delete-personal-information').on('click', function() {
                ...
                GeneralFunctions.displayMessageBox(WBLang.delete_personal_information,
                    WBLang.delete_personal_information_prompt, buttons);
            });
        }
    ...
    exports.updateConfirmFrame = function() {
        ...
        var html =
            '<h3 class="booking-heading">' + $('#select-service option:selected').text() + '</h3>' +
            '<p>' +
        ...
        var idNum = GeneralFunctions.escapeHtml($('#id-num').val());
        var dobNum = GeneralFunctions.escapeHtml($('#dob-num').val());
        var phoneNumber = GeneralFunctions.escapeHtml($('#phone-number').val());
        var cellNumber = GeneralFunctions.escapeHtml($('#cell-number').val());
        var email = GeneralFunctions.escapeHtml($('#email').val());
        var address = GeneralFunctions.escapeHtml($('#address').val());
        //var city = GeneralFunctions.escapeHtml($('#city').val());
        var zipCode = GeneralFunctions.escapeHtml($('#zip-code').val());
        // MA-Details
        var mafirstName = GeneralFunctions.escapeHtml($('#mafirst-name').val());
        var malastName = GeneralFunctions.escapeHtml($('#malast-name').val());
        var maComp = GeneralFunctions.escapeHtml($('#macomp').val());
        //var maidNum = GeneralFunctions.escapeHtml($('#maid-num').val());
        //var maNum = GeneralFunctions.escapeHtml($('#ma-num').val());
        var maEmail = GeneralFunctions.escapeHtml($('#maemail').val());
        var mamobileNumber = GeneralFunctions.escapeHtml($('#mamobile-number').val());
      //  var maworkNumber = GeneralFunctions.escapeHtml($('#mawork-number').val());
      //  var mahomeNumber = GeneralFunctions.escapeHtml($('#mahome-number').val());

      //  var maaddress = GeneralFunctions.escapeHtml($('#maaddress').val());
      //  var macity = GeneralFunctions.escapeHtml($('#macity').val());
      //  var mazipCode = GeneralFunctions.escapeHtml($('#mazip-code').val());

        html =

            '<h4 class="booking_sub_headings">' + firstName + ' ' + lastName + '</h4>' +
            '<p>' +
            WBLang.id_num + ': ' + idNum +
            '<br/>' +
            WBLang.dob_num + ': ' + dobNum +
            '<br/>' +
            WBLang.phone_number + ': ' + phoneNumber +
            '<br/>' +
            WBLang.cell_number + ': ' + cellNumber +
            '<br/>' +
            WBLang.email + ': ' + email +
            '<br/>' +
            WBLang.address + ': ' + address +
            '<br/>' +
          //  WBLang.city + ': ' + city +
          //  '<br/>' +
            WBLang.zip_code + ': ' + zipCode +
            '<br/>' +
            WBLang.ma_first_name + ': ' + mafirstName +
            '<br/>' +
            WBLang.ma_last_name + ': ' + malastName +
            '<br/>' +
            WBLang.medical_aid + ': ' + maComp +
            '<br/>' +
      //      WBLang.ma_id_num + ': ' + maidNum +
      //      '<br/>' +
    //        WBLang.ma_num + ': ' + maNum +
    //        '<br/>' +
            WBLang.ma_email + ': ' + maEmail +
          '<br/>' +
            WBLang.ma_mobile_number + ': ' + mamobileNumber +
            '<br/>' +
    //        WBLang.ma_work_number + ': ' + maworkNumber +
    //        '<br/>' +
    //        WBLang.ma_home_number + ': ' + mahomeNumber +
    //        '<br/>' +
    //        WBLang.ma_address + ': ' + maAddress +
    //        '<br/>' +
    //        WBLang.ma_city + ': ' + maCity +
    //        '<br/>' +
    //        WBLang.ma_state + ': ' + maState +
    //        '<br/>' +
    //        WBLang.ma_zip_code + ': ' + mazipCode +
            '</p>';
        ...
        postData.customer = {
          ...
          //  city: $('#city').val(),
            ...
            ma_first_name: $('#mafirst-name').val(),
            ma_last_name: $('#malast-name').val(),
            medical_aid: $('#maComp').val(),
    //        ma_id_num: $('#maid-num').val(),
    //        ma_num: $('#ma-num').val(),
            ma_email: $('#maemail').val(),
            ma_mobile_number: $('#mamobile-number').val(),
    //        ma_work_number: $('#mawork-number').val(),
    //        ma_home_number: $('#mahome-number').val(),
    //        ma_address: $('#ma-address').val(),
    //        ma_city: $('#ma-city').val(),
    //        ma_state: $('#ma-state').val(),
    //        ma_zip_code: $('#mazip-code').val()
        };
    ...
    function _applyAppointmentData(appointment, provider, customer) {
        try {
            ...
            $('#id-num').val(customer.id_num);
            $('#dob-num').val(customer.dob_num);
        ...
        $.each(GlobalVariables.availableServices, function(index, service) {
            if (service.id == serviceId) { // Just found the service.
                ...
                if (service.duration != '' && service.duration != null) {
                    html += '<strong>[ ' + WBLang.duration + ' ' + service.duration + ' ' + WBLang.minutes + ' ]</strong>';
                }
