    ...
    function handleAuthResult(authResult) {
        try {
            ...
            gapi.client.load('calendar', 'v3', function () {
                ...
                request.execute(function (response) {
                    ...
                    $('#success-frame').append(
                        ...
                        '<h4>' + WBLang.success + '</h4>' +
                        '<p>' +
                        WBLang.appointment_added_to_google_calendar +
                        '<br>' +
                        '<a href="' + response.htmlLink + '" target="_blank">' +
                        WBLang.view_appointment_in_google_calendar +
                        '</a>' +
                        '</p>' +
                        '</div>'
                    );
            ...
            $('#success-frame').append(
                ...
                '<h4>' + WBLang.oops_something_went_wrong + '</h4>' +
                '<p>' +
                WBLang.could_not_add_to_google_calendar +
                '<pre>' + exc + '</pre>' +
                '</p>' +
                '</div>');
        }
