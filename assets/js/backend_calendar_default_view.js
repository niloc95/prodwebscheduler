...
(function (exports) {
    ...
    function _bindEventHandlers() {
        ...
        $calendarPage.on('click', '.edit-popover', function () {
            ...
            if (lastFocusedEventData.data.is_unavailable == false) {
                ...
                $dialog.find('.modal-header h3').text(WBLang.edit_appointment_title);
                ...
                $dialog.find('#medical-aid').val(customer.medical_aid);
        ...
        $calendarPage.on('click', '.delete-popover', function () {
            ...
            if (lastFocusedEventData.data.is_unavailable == false) {
                ...
                GeneralFunctions.displayMessageBox(WBLang.delete_appointment_title,
        ...
        $('#select-filter-item').change(function () {
            ...
            if ($('#select-filter-item option:selected').attr('type') === FILTER_TYPE_SERVICE) {
                ...
                if ($('#select-filter-item option:selected').attr('google-sync') === 'true') {
                    ...
                    $('#enable-sync span:eq(1)').text(WBLang.disable_sync);
                    $('#google-sync').prop('disabled', false);
                } else {
                    $('#enable-sync').removeClass('btn-danger enabled');
                    $('#enable-sync span:eq(1)').text(WBLang.enable_sync);
    ...
    function _calendarEventClick(event, jsEvent, view) {
        ...
        if ($(this).hasClass('fc-unavailable') || $parent.hasClass('fc-unavailable') || $altParent.hasClass('fc-unavailable')) {
            ...
            html =
                ...
                + '</style>' +
                '<strong>' + WBLang.start + '</strong> '
                + GeneralFunctions.formatDate(event.start.format('YYYY-MM-DD HH:mm:ss'), GlobalVariables.dateFormat, true)
                + '<br>' +
                '<strong>' + WBLang.end + '</strong> '
                + GeneralFunctions.formatDate(event.end.format('YYYY-MM-DD HH:mm:ss'), GlobalVariables.dateFormat, true)
                + '<br>'
                + notes
                + '<hr>' +
                '<center>' +
                '<button class="edit-popover btn btn-primary ' + displayEdit + '">' + WBLang.edit + '</button>' +
                '<button class="delete-popover btn btn-danger ' + displayDelete + '">' + WBLang.delete + '</button>' +
                '<button class="close-popover btn btn-default" data-po=' + jsEvent.target + '>' + WBLang.close + '</button>' +
                '</center>';
        } else {
            ...
            html =
                ...
                + GeneralFunctions.formatDate(event.end.format('YYYY-MM-DD HH:mm:ss'), GlobalVariables.dateFormat, true)
                + '<br>' +
                '<strong>' + WBLang.service + '</strong> '
                + event.data.service.name
                + '<br>' +
                '<strong>' + WBLang.provider + '</strong> '
                + event.data.provider.first_name + ' '
                + event.data.provider.last_name
                + '<br>' +
                '<strong>' + WBLang.customer + '</strong> '
                + event.data.customer.first_name + ' '
                + event.data.customer.last_name
                + '<br>' +
                '<strong>' + WBLang.email + '</strong> '
                + event.data.customer.email
                + '<br>' +
                '<strong>' + WBLang.phone_number + '</strong> '
    ...
    function _calendarEventResize(event, delta, revertFunc, jsEvent, ui, view) {
        if (GlobalVariables.user.privileges.appointments.edit == false) {
            revertFunc();
            Backend.displayNotification(WBLang.no_privileges_edit_appointments);
            return;
        }
        ...
        if (event.data.is_unavailable == false) {
            ...
            var successCallback = function (response) {
                ...
                Backend.displayNotification(WBLang.appointment_updated, [
                    {
                        'label': 'Undo',
            ...
            var successCallback = function (response) {
                ...
                Backend.displayNotification(WBLang.unavailable_updated, [
                    {
                        'label': 'Undo',
                        'function': undoFunction
                    }
                        ...
                        switch (calendarView) {
                            case 'agendaDay':
                                ...
                                $.each(response.unavailables, function (index, unavailable) {
                                    ...
                                    var unavailablePeriod = {
                                        title: WBLang.unavailable + notes,
    ...
    exports.initialize = function () {
        ...
        $('#calendar').fullCalendar({
            ...
            monthNames: [WBLang.january, WBLang.february, WBLang.march, WBLang.april,
                WBLang.may, WBLang.june, WBLang.july, WBLang.august,
                WBLang.september, WBLang.october, WBLang.november,
                WBLang.december],
            monthNamesShort: [WBLang.january.substr(0, 3), WBLang.february.substr(0, 3),
                WBLang.march.substr(0, 3), WBLang.april.substr(0, 3),
                WBLang.may.substr(0, 3), WBLang.june.substr(0, 3),
                WBLang.july.substr(0, 3), WBLang.august.substr(0, 3),
                WBLang.september.substr(0, 3), WBLang.october.substr(0, 3),
                WBLang.november.substr(0, 3), WBLang.december.substr(0, 3)],
        ...
        if (GlobalVariables.availableProviders.length > 0) {
            var optgroupHtml = '<optgroup label="' + WBLang.providers + '" type="providers-group">';
        ...
        if (GlobalVariables.availableServices.length > 0) {
            optgroupHtml = '<optgroup label="' + WBLang.services + '" type="services-group">';
