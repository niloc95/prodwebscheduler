...
class Admins_Model extends CI_Model {
    ...
    public function exists($admin)
    {
        ...
        $num_rows = $this->db
            ...
            ->where('wb_users.email', $admin['email'])
            ->where('wb_roles.slug', DB_SLUG_ADMIN)
            ->get()->num_rows();
    ...
    protected function _insert($admin)
    {
        ...
        if ( ! $this->db->insert('wb_users', $admin))
        {
        ...
        if ( ! $this->db->insert('wb_user_settings', $settings))
        {
    ...
    public function validate($admin)
    {
        ...
        if (isset($admin['id']))
        {
            $num_rows = $this->db->get_where('wb_users', ['id' => $admin['id']])
                ->num_rows();
            if ($num_rows == 0)
            {
    ...
    public function delete($admin_id)
    {
        ...
        $admin_count = $this->db->get_where('wb_users',
        ...
        $num_rows = $this->db->get_where('wb_users', ['id' => $admin_id])->num_rows();
        if ($num_rows == 0)
        {
            return FALSE; // Record does not exist in database.
        }

        return $this->db->delete('wb_users', ['id' => $admin_id]);
    }
    ...
    public function get_row($admin_id)
    {
        ...
        if ($this->db->get_where('wb_users', ['id' => $admin_id])->num_rows() == 0)
        {
            throw new Exception('The given admin id does not match a record in the database.');
        }

        $admin = $this->db->get_where('wb_users', ['id' => $admin_id])->row_array();

        $admin['settings'] = $this->db->get_where('wb_user_settings',
    ...
    public function get_value($field_name, $admin_id)
    {
        ...
        $result = $this->db->get_where('wb_users', ['id' => $admin_id]);
        if ($result->num_rows() == 0)
        {
    ...
    public function get_batch($where_clause = '')
    {
        ...
        $batch = $this->db->get_where('wb_users', ['id_roles' => $role_id])->result_array();
    ...
    {
        return (int)$this->db->get_where('wb_roles', ['slug' => DB_SLUG_ADMIN])->row()->id;
    }
