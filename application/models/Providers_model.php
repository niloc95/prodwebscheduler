 ...
 *
 * Contains the database operations for the service provider users of @WebScheduler.
 *
 * Data Structure:
 *      'fist_name'
 *      'last_name' (required)
...
class Providers_Model extends CI_Model {
    ...
    public function exists($provider)
    {
        ...
        $num_rows = $this->db
            ...
            ->where('wb_users.email', $provider['email'])
            ->where('wb_roles.slug', DB_SLUG_PROVIDER)
            ->get()->num_rows();
    ...
    protected function _insert($provider)
    {
        ...
        if ( ! $this->db->insert('wb_users', $provider))
        {
    ...
    protected function _update($provider)
    {
        ...
        {
            $salt = $this->db->get_where('wb_user_settings', ['id_users' => $provider['id']])->row()->salt;
            $settings['password'] = hash_password($salt, $settings['password']);
        }

        // Update provider record.
        $this->db->where('id', $provider['id']);
        if ( ! $this->db->update('wb_users', $provider))
        {
    ...
    public function validate($provider)
    {
        ...
        if (isset($provider['id']))
        {
            $num_rows = $this->db->get_where('wb_users',
    ...
    public function delete($provider_id)
    {
        ...
        $num_rows = $this->db->get_where('wb_users', ['id' => $provider_id])->num_rows();
        if ($num_rows == 0)
        {
            return FALSE; // Record does not exist in database.
        }

        return $this->db->delete('wb_users', ['id' => $provider_id]);
    }
    ...
    public function get_row($provider_id)
    {
        ...
        if ($this->db->get_where('wb_users', ['id' => $provider_id])->num_rows() == 0)
        {
            throw new Exception('Selected record does not exist in the database.');
        }

        // Get provider data.
        $provider = $this->db->get_where('wb_users', ['id' => $provider_id])->row_array();


        // Include provider services.
        $services = $this->db->get_where('wb_services_providers',
        ...
        $provider['settings'] = $this->db->get_where('wb_user_settings',
    ...
    public function get_value($field_name, $provider_id)
    {
        ...
        $result = $this->db->get_where('wb_users', ['id' => $provider_id]);
        if ($result->num_rows() == 0)
        {
    ...
    public function get_batch($where_clause = '')
    {
        ...
        $batch = $this->db->get_where('wb_users',
    ...
    {
        return $this->db->get_where('wb_roles', ['slug' => DB_SLUG_PROVIDER])->row()->id;
    }
    ...
    {
        $provider_settings = $this->db->get_where('wb_user_settings', ['id_users' => $provider_id])->row_array();
    ...
    public function set_setting($setting_name, $value, $provider_id)
    {
        ...
        return $this->db->update('wb_user_settings', [$setting_name => $value]);
    }
    ...
    protected function save_settings($settings, $provider_id)
    {
        ...
        if ($this->db->get_where('wb_user_settings', ['id_users' => $provider_id])
                ->num_rows() == 0)
        {
            $this->db->insert('wb_user_settings', ['id_users' => $provider_id]);
        }
    ...
    protected function save_services($services, $provider_id)
    {
        ...
        $this->db->delete('wb_services_providers', ['id_users' => $provider_id]);
        foreach ($services as $service_id)
        {
            ...
            $this->db->insert('wb_services_providers', $service_provider);
        }
    }
    ...
    {
        $num_rows = $this->db->get_where('wb_user_settings',
