...
class Appointments_Model extends CI_Model {
    ...
    protected function _insert($appointment)
    {
        ...
        if ( ! $this->db->insert('wb_appoinments', $appointment))
        {
    ...
    protected function _update($appointment)
    {
        ...
        if ( ! $this->db->update('wb_appoinments', $appointment))
        {
    ...
    public function validate($appointment)
    {
        ...
        $num_rows = $this->db
            ...
            ->where('wb_users.id', $appointment['id_users_provider'])
        ...
        if ($appointment['is_unavailable'] == FALSE)
        {
            ...
            $num_rows = $this->db
                ...
                ->from('wb_users')
                ->join('wb_roles', 'wb_roles.id = wb_users.id_roles', 'inner')
                ->where('wb_users.id', $appointment['id_users_customer'])
                ->where('wb_roles.slug', DB_SLUG_CUSTOMER)
                ->get()->num_rows();
            if ($num_rows == 0)
            {
    ...
    public function delete($appointment_id)
    {
        ...
        $num_rows = $this->db->get_where('wb_appoinments', ['id' => $appointment_id])->num_rows();
    ...
    public function get_row($appointment_id)
    {
        ...
        return $this->db->get_where('wb_appoinments', ['id' => $appointment_id])->row_array();
    }
    ...
    public function get_value($field_name, $appointment_id)
    {
        ...
        if ($this->db->get_where('wb_appoinments', ['id' => $appointment_id])->num_rows() == 0)
        {
            throw new Exception('The record with the provided id '
                . 'does not exist in the database: ' . $appointment_id);
        }

        $row_data = $this->db->get_where('wb_appoinments', ['id' => $appointment_id])->row_array();
    ...
    public function get_batch($where_clause = '', $aggregates = FALSE)
    {
        ...
        $appointments = $this->db->get('wb_appoinments')->result_array();
    ...
    public function add_unavailable($unavailable)
    {
        ...
        $where_clause = [
            ...
            'id_roles' => $this->db->get_where('wb_roles', ['slug' => DB_SLUG_PROVIDER])->row()->id
        ];

        if ($this->db->get_where('wb_users', $where_clause)->num_rows() == 0)
        {
        ...
        if ( ! isset($unavailable['id']))
        {
            ...
            $this->db->insert('wb_appoinments', $unavailable);
            $unavailable['id'] = $this->db->insert_id();
        }
        else
        {
            $this->db->where(['id' => $unavailable['id']]);
            $this->db->update('wb_appoinments', $unavailable);
        }
    ...
    public function delete_unavailable($unavailable_id)
    {
        ...
        $num_rows = $this->db->get_where('wb_appoinments', ['id' => $unavailable_id])->num_rows();
    ...
    public function clear_google_sync_ids($provider_id)
    {
        ...
        $this->db->update('wb_appoinments', ['id_google_calendar' => NULL],
    ...
    {
        $appointment['service'] = $this->db->get_where('wb_services',
            ['id' => $appointment['id_services']])->row_array();
        $appointment['provider'] = $this->db->get_where('wb_users',
            ['id' => $appointment['id_users_provider']])->row_array();
        $appointment['customer'] = $this->db->get_where('wb_users',
