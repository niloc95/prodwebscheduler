...
class Settings_Model extends CI_Model {
    ...
    public function get_setting($name)
    {
        ...
        if ($this->db->get_where('wb_settings', ['name' => $name])->num_rows() == 0)
        { // Check if setting exists in db.
            throw new Exception('$name setting does not exist in database: ' . $name);
        }

        $query = $this->db->get_where('wb_settings', ['name' => $name]);
    ...
    public function set_setting($name, $value)
    {
        ...
        {
            // Update setting
            if ( ! $this->db->update('wb_settings', ['value' => $value], ['name' => $name]))
            {
                throw new Exception('Could not update database setting.');
            }
            $setting_id = (int)$this->db->get_where('wb_settings', ['name' => $name])->row()->id;
        }
        else
        {
            // Insert setting
            ...
            if ( ! $this->db->insert('wb_settings', $insert_data))
            {
    ...
    public function remove_setting($name)
    {
        ...
        return $this->db->delete('wb_settings', ['name' => $name]);
    }
    ...
    public function save_settings($settings)
    {
        ...
        foreach ($settings as $setting)
        {
            ...
            if ( ! $this->db->update('wb_settings', ['value' => $setting['value']]))
            {
     ...
     *
     * @return array Array of all the system settings stored in the 'wb_settings' table.
     */
    public function get_settings()
    {
        return $this->db->get('wb_settings')->result_array();
    }
}
