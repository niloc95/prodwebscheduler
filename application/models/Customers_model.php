...
class Customers_Model extends CI_Model {
    ...
    public function exists($customer)
    {
        ...
        $num_rows = $this->db
            ...
            ->where('wb_users.email', $customer['email'])
    ...
    protected function _insert($customer)
    {
        ...
        if ( ! $this->db->insert('wb_users', $customer))
        {
    ...
    protected function _update($customer)
    {
        ...
        if ( ! $this->db->update('wb_users', $customer))
        {
    ...
    public function delete($customer_id)
    {
        ...
        $num_rows = $this->db->get_where('wb_users', ['id' => $customer_id])->num_rows();
        if ($num_rows == 0)
        {
            return FALSE;
        }

        return $this->db->delete('wb_users', ['id' => $customer_id]);
    }
    ...
    public function get_row($customer_id)
    {
        ...
        return $this->db->get_where('wb_users', ['id' => $customer_id])->row_array();
    }
    ...
    public function get_value($field_name, $customer_id)
    {
        ...
        if ($this->db->get_where('wb_users', ['id' => $customer_id])->num_rows() == 0)
        {
            throw new Exception('The record with the $customer_id argument '
                . 'does not exist in the database: ' . $customer_id);
        }

        $row_data = $this->db->get_where('wb_users', ['id' => $customer_id]
        )->row_array();
        ...
        $customer = $this->db->get_where('wb_users', ['id' => $customer_id])->row_array();
    ...
    public function get_batch($where_clause = '')
    {
        ...
        return $this->db->get('wb_users')->result_array();
    }
    ...
    {
        return $this->db->get_where('wb_roles', ['slug' => DB_SLUG_CUSTOMER])->row()->id;
    }
}
