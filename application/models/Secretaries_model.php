...
class Secretaries_Model extends CI_Model {
    ...
    public function exists($secretary)
    {
        ...
        $num_rows = $this->db
            ...
            ->where('wb_users.email', $secretary['email'])
            ->where('wb_roles.slug', DB_SLUG_SECRETARY)
            ->get()->num_rows();
    ...
    protected function _insert($secretary)
    {
        ...
        if ( ! $this->db->insert('wb_users', $secretary))
        {
    ...
    protected function _update($secretary)
    {
        ...
        {
            $salt = $this->db->get_where('wb_user_settings', ['id_users' => $secretary['id']])->row()->salt;
            $settings['password'] = hash_password($salt, $settings['password']);
        }

        $this->db->where('id', $secretary['id']);
        if ( ! $this->db->update('wb_users', $secretary))
        {
    ...
    public function validate($secretary)
    {
        ...
        if (isset($secretary['id']))
        {
            $num_rows = $this->db->get_where('wb_users', ['id' => $secretary['id']])
                ->num_rows();
            if ($num_rows == 0)
            {
        ...
        $num_rows = $this->db
            ...
            ->where('wb_roles.slug', DB_SLUG_SECRETARY)
            ->where('wb_users.email', $secretary['email'])
            ->where('wb_users.id <>', $secretary_id)
            ->get()
            ->num_rows();
    ...
    public function delete($secretary_id)
    {
        ...
        $num_rows = $this->db->get_where('wb_users', ['id' => $secretary_id])->num_rows();
        if ($num_rows == 0)
        {
            return FALSE; // Record does not exist in database.
        }

        return $this->db->delete('wb_users', ['id' => $secretary_id]);
    }
    ...
    public function get_row($secretary_id)
    {
        ...
        if ($this->db->get_where('wb_users', ['id' => $secretary_id])->num_rows() == 0)
        {
            throw new Exception('The given secretary id does not match a record in the database.');
        }

        $secretary = $this->db->get_where('wb_users', ['id' => $secretary_id])->row_array();

        $secretary_providers = $this->db->get_where('wb_secretaries_providers',
        ...
        $secretary['settings'] = $this->db->get_where('wb_user_settings',
    ...
    public function get_value($field_name, $secretary_id)
    {
        ...
        $result = $this->db->get_where('wb_users', ['id' => $secretary_id]);
        if ($result->num_rows() == 0)
        {
    ...
    public function get_batch($where_clause = '')
    {
        ...
        $batch = $this->db->get('wb_users')->result_array();
    ...
    {
        return (int)$this->db->get_where('wb_roles', ['slug' => DB_SLUG_SECRETARY])->row()->id;
    }
    ...
    protected function save_providers($providers, $secretary_id)
    {
        ...
        $this->db->delete('wb_secretaries_providers', ['id_users_secretary' => $secretary_id]);

        if (count($providers) > 0)
        {
            foreach ($providers as $provider_id)
            {
                $this->db->insert('wb_secretaries_providers', [
    ...
    protected function save_settings($settings, $secretary_id)
    {
        ...
        if ($num_rows == 0)
        {
            $this->db->insert('wb_user_settings', ['id_users' => $secretary_id]);
        }
    ...
    {
        $provider_settings = $this->db->get_where('wb_user_settings',
