...
class Services_Model extends CI_Model {
    ...
    {
        if ( ! $this->db->insert('wb_services', $service))
        {
    ...
    protected function _update($service)
    {
        ...
        if ( ! $this->db->update('wb_services', $service))
        {
    ...
    public function exists($service)
    {
        ...
        $num_rows = $this->db->get_where('wb_services', [
            'name' => $service['name'],
    ...
    public function validate($service)
    {
        ...
        if (isset($service['id']))
        {
            $num_rows = $this->db->get_where('wb_services', ['id' => $service['id']])
                ->num_rows();
            if ($num_rows == 0)
            {
        ...
        {
            $num_rows = $this->db->get_where('wb_service_categories',
    ...
    public function find_record_id($service)
    {
        ...
        $result = $this->db->get_where('wb_services', [
            'name' => $service['name'],
            'duration' => $service['duration'],
            'price' => $service['price']
        ]);
    ...
    public function delete($service_id)
    {
        ...
        $num_rows = $this->db->get_where('wb_services', ['id' => $service_id])->num_rows();
        if ($num_rows == 0)
        {
            return FALSE; // Record does not exist
        }

        return $this->db->delete('wb_services', ['id' => $service_id]);
    }
    ...
    public function get_row($service_id)
    {
        ...
        return $this->db->get_where('wb_services', ['id' => $service_id])->row_array();
    }
    ...
    public function get_batch($where_clause = NULL)
    {
        ...
        return $this->db->get('wb_services')->result_array();
    }
    ...
    public function get_available_services()
    {
        $this->db->distinct();
        return $this->db
            ->select('wb_services.*, wb_service_categories.name AS category_name, '
                . 'wb_service_categories.id AS category_id')
            ->from('wb_services')
            ->join('wb_services_providers',
                'wb_services_providers.id_services = wb_services.id', 'inner')
            ->join('wb_service_categories',
                'wb_service_categories.id = wb_services.id_service_categories', 'left')
            ->get()->result_array();
    }
    ...
    public function add_category($category)
    {
        ...
        {
            $this->db->insert('wb_service_categories', $category);
            $category['id'] = $this->db->insert_id();
        }
        else
        {
            $this->db->where('id', $category['id']);
            $this->db->update('wb_service_categories', $category);
        }
    ...
    public function delete_category($category_id)
    {
        ...
        $num_rows = $this->db->get_where('wb_service_categories', ['id' => $category_id])
            ->num_rows();
        if ($num_rows == 0)
        {
        ...
        return $this->db->delete('wb_service_categories');
    }
    ...
    public function get_category($category_id)
    {
        ...
        $result = $this->db->get_where('wb_service_categories', ['id' => $category_id]);
    ...
    public function get_all_categories($where = '')
    {
        ...
        return $this->db->get('wb_service_categories')->result_array();
    }
